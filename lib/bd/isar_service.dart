...
class IsarService {
  ...
  Future<dynamic> isarConfig() async {
    ...
    final isar =
        Isar.open([UserEntitySchema], directory: dir.path, inspector: true);
  }

  Future<int> findUsers(String tabNum) async {
    final isar = await db;
    final token = await isar.userEntitys.filter().tabNumberEqualTo(tabNum);
    int res = 0;
  ...
  Future<int> findUserPass(String value_tab2, String password) async {
    final isar = await db;
    final token = await isar.userEntitys.filter().tabNumberEqualTo(value_tab2);
    final parol = token.passEqualTo(password);
    int res = 0;
    if (parol.isEmptySync()) {
      res = 1;
    } else {
      res = 0;
    }
  ...
  Future<void> addUser(
    ...
    Future<void> addUserPass(String pass) async {
      final isar = await db;
      final newUser = UserEntity()..pass = pass;
      //tabNumber => string
      await isar.writeTxn(() async {
        await isar.userEntitys.put(newUser);
      });
